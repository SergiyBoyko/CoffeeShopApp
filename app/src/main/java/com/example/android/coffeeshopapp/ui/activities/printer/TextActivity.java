package com.example.android.coffeeshopapp.ui.activities.printer;
...
import com.example.android.coffeeshopapp.utils.printer.AidlUtil;
import com.example.android.coffeeshopapp.utils.printer.BluetoothUtil;
import com.example.android.coffeeshopapp.utils.printer.ESCUtil;
...
/**
 * Created by dev_serhii on 06.01.2018.
 */
...
public class TextActivity extends BaseActivity implements CompoundButton.OnCheckedChangeListener {
    ...
    private int record;
    private boolean isBold, isUnderLine, isBluetoothConnected;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String message = getIntent().getStringExtra(Constants.PRINT_TEXT_EXTRA);
        ...
        findViewById(R.id.text_character).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                builder.setTitle(getResources().getString(R.string.characterset))
                        .setItems(mStrings, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialogInterface, int i) {
                                mTextView1.setText(mStrings[i]);
                                record = i;
                            }
                        });
                builder.setNegativeButton(getResources().getString(R.string.cancel), (dialog, whichButton) -> {
        ...
        isBluetoothConnected = BluetoothUtil.connectBlueTooth(TextActivity.this);
        if (isBluetoothConnected) Toast.makeText(this, "bluetooth printer connected", Toast.LENGTH_SHORT).show();
    ...
    public void onClick(View view) {
        ...
        alert.setTitle("Choose print method");
        ...
        alert.setNeutralButton("Bluetooth", (dialogInterface, i) -> {
            if (!isBluetoothConnected) {
                isBluetoothConnected = BluetoothUtil.connectBlueTooth(this);
                if (!isBluetoothConnected) return;
            }
            printByBluTooth(content);
        });

        alert.setPositiveButton(getResources().getString(R.string.close_receipt),
                (dialog, whichButton) -> {
                    // what ever you want to do with No option.
                });
